...
final class PaiementController extends AbstractController
{
    ...
    #[Route('/paiement/complement/{id}', name: 'app_payer_complement', methods: ['POST'])]
    public function payerComplement(Reservation $reservation, Request $request, ComplementService $complements): Response
    {
        $user = $this->getUser();
        if (!$user instanceof Utilisateur || $user->getRoleMetier() !== UserRole::CLIENT || $reservation->getUtilisateur() !== $user) { throw $this->createAccessDeniedException(); }
        if (!$this->isCsrfTokenValid('payer-complement-'.$reservation->getId(), (string) $request->request->get('_token'))) { throw $this->createAccessDeniedException(); }
        try { $complements->payer($reservation); $this->addFlash('success', 'Complément d’annulation payé en ligne.'); }
        catch (RegleMetierException $e) { $this->addFlash('error', $e->getMessage()); }
        return $this->redirectToRoute('app_compte');
    }
}
