...
{
    public function listerReservations(Utilisateur $utilisateur, array $reservations): array { if (in_array($utilisateur->getRole(), ['employe', 'administrateur'], true)) { return $reservations; } return array_values(array_filter($reservations, fn (Reservation $r) => $r->getUtilisateur() === $utilisateur)); }
    ...
    public function messageListeVide(array $reservations): ?string { return $reservations === [] ? 'aucune réservation' : null; }
}
